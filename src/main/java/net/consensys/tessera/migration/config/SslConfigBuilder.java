...
public class SslConfigBuilder {
    ...
    public SslConfig build() {
        ...
        if(Stream.of(serverKeyStore,clientKeyStore,tlsservertrust)
                .allMatch(o -> Objects.isNull(o))) {
            return null;
        }
        ...
        Optional.ofNullable(serverKeyStore)
                .map(Paths::get)
                .ifPresent(sslConfig::setServerKeyStore);
        ...
        Optional.ofNullable(clientKeyStore)
                .map(Paths::get)
                .ifPresent(sslConfig::setClientKeyStore);

        Optional.ofNullable(clientTlsCertificatePath)
                .map(Paths::get)
                .ifPresent(sslConfig::setClientTlsCertificatePath);
        ...
        Optional.ofNullable(knownServersFilePath)
                .map(Paths::get)
                .ifPresent(sslConfig::setKnownServersFile);

        Optional.ofNullable(knownClientFilePath)
                .map(Paths::get)
                .ifPresent(sslConfig::setKnownClientsFile);

        Optional.ofNullable(serverTlsCertificatePath)
                .map(Paths::get)
                .ifPresent(sslConfig::setServerTlsCertificatePath);
